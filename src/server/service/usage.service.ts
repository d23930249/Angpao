...
export async function getUsageStats(): Promise<UsageStats> {
  const [wallets] = await rows(
    sql`select count(distinct public_key)::int c from angpao_sessions`,
  );
  const [logins] = await rows(sql`select count(*)::int c from angpao_sessions`);
  const [gifts] = await rows(sql`select count(*)::int c from angpao_gifts`);
  const [senders] = await rows(
    sql`select count(distinct sender_public_key)::int c from angpao_gifts`,
  );
